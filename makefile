...
LFLAGS = -mcmodel=medium


FILES = dimensions.f90 mult_proc.f90 grid.f90 var_arrays.f90 inputs.f90 misc.f90 boundary.f90 grid_interp.f90 gutsf.f90 gutsp.f90 initial.f90 part_init.f90 maind.f90
FILESO = dimensions.f90 boundary.f90 grid_interp.f90 mult_proc.f90 var_arrays.f90 inputs.f90 grid.f90 initial.f90 gutsf.f90 misc.f90 part_init.f90 gutsp.f90 maind.f90
...
INCLUDE4 = dimensions.o inputs.o grid.o mult_proc.o boundary.o misc.o grid_interp.o gutsp.o gutsf.o initial.o part_init.o var_arrays.o
OBJECTS = dimensions.o inputs.o grid.o mult_proc.o boundary.o misc.o grid_interp.o gutsp.o gutsf.o initial.o part_init.o var_arrays.o maind.o
MODS = dimensions.mod mult_proc.mod var_array.mod inputs.mod grid.mod initial.mod gutsf.mod misc.mod boundary.mod part_init.mod grid_interp.mod gutsp.mod

ifeq ($(DEBUG),1)
	CFLAGS += -Wall -Wno-unused-variable -g3 -Og -fbounds-check -ffpe-trap=invalid,zero,overflow
	MPI = mpifort-vt
	LINK = $(MPI)
else
	CFLAGS += -O3 -flto
	LFLAGS += -O3 -flto
	MPI = mpifort
	LFLAGS += -static-libgfortran -static-libgcc -I/usr/local/include -pthread -I/usr/local/lib -Wl,-rpath -Wl,/usr/local/lib -Wl,--enable-new-dtags -L/usr/local/lib -Wl,-Bstatic -lmpi_usempi -lmpi_mpifh -lmpi -lopen-rte -lopen-pal -lm -lrt -lutil
	LINK = $(FORT)
endif
F90 = $(MPI) $(CFLAGS)
